...
def save_to_minio(bucket_name, file_name, data):
    ...
    with open('/tmp/' + file_name, mode='w', newline='') as csv_file:  # Salvar em um arquivo temporário
        writer = csv.DictWriter(csv_file, fieldnames=data[0].keys())
        writer.writeheader()
        writer.writerows(data)

    # Fazer upload do arquivo CSV para o MinIO
    key = f"dataway/protheus/clients/{file_name}"
    with open('/tmp/' + file_name, 'rb') as csv_file:
...
if __name__ == "__main__":
    num_records = random.randint(1_000, 10_000)
    print(f"Número de registros a serem gerados: {num_records}")
    fake_data = generate_fake_data(num_records)
    
    file_name = f"clients_data_{int(time.time())}.csv" 
    
    save_to_minio("landing-zone", file_name, fake_data)
